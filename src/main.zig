...
//!
//! ```
//! fdm [url ...] [-n segments] [--stall ms] [--retries n] [--db file]
//! ```
//!
...
pub fn main(init: std.process.Init) !void {
    ...
    while (i < args.len) : (i += 1) {
        const a = args[i];
        if (std.mem.eql(u8, a, "-n") or std.mem.eql(u8, a, "--stall") or std.mem.eql(u8, a, "--retries") or std.mem.eql(u8, a, "--db")) {
...
fn usage() error{Usage} {
    std.debug.print(
        \\usage: fdm [url ...] [-n segments] [--stall ms] [--retries n] [--db file]
        \\
    , .{});
    return error.Usage;
}
